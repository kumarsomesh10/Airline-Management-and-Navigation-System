...
public class FlightReservation implements DisplayClass {
    ...
    void bookFlight(String flightNo, int numOfTickets, String userID) {
        ...
        if (!isFound) {
            User.raiseError("Invalid Flight Number...!\nNo flight with the  ID \"" + flightNo + "\" was found...");
        } else {
            User.addToMessage("You've booked "+numOfTickets+" tickets for Flight \""+flightNo.toUpperCase()+"\"...");
        }
    }
        ...
        for (Customer customer : Customer.customerCollection) {
            if (userID.equals(customer.getUserID())) {
                if (customer.getFlightsRegisteredByUser().size() != 0) {
                    User.addToMessage("Here is the list of all the Flights registered by you");
                    displayFlightsRegisteredByOneUser(userID);
                    User.addToMessage("Enter the Flight Number of the Flight you want to cancel : ");
                    flightNum = User.getStrInput(User.message);
                    User.addToMessage("Enter the number of tickets to cancel : ");
                    int numOfTickets = User.getIntInput(User.message);
                    ...
                    while (flightIterator.hasNext()) {
                        ...
                        if (flightNum.equalsIgnoreCase(flight.getFlightNumber())) {
                            ...
                            while (numOfTickets > numOfTicketsForFlight) {
                                User.addToMessage("ERROR!!! Number of tickets cannot be greater than " + numOfTicketsForFlight + " for this flight. Please enter the number of tickets again : ");
                                numOfTickets = User.getIntInput(User.message);
                            }
                    ...
                    User.raiseError("No Flight Has been Registered by you with ID \"\"" + flightNum.toUpperCase() +"\"\".....");
                }
//                index++;
                if (!isFound) {
                    User.raiseError("ERROR!!! Couldn't find Flight with ID \"" + flightNum.toUpperCase() + "\".....");
                }
            }
        }
    }
    ...
    public void displayFlightsRegisteredByOneUser(String userID) {
        User.addToMessage("| Num | FLIGHT SCHEDULE | FLIGHT NO |  Booked Tickets  | FROM ====>>       | ====>> TO |    ARRIVAL TIME     | FLIGHT TIME |  GATE  |  FLIGHT STATUS  |");
        for (Customer customer : Customer.customerCollection) {
            List<Flight> f = customer.getFlightsRegisteredByUser();
            int size = customer.getFlightsRegisteredByUser().size();
            if (userID.equals(customer.getUserID())) {
                for (int i = 0; i < size; i++) {
                    User.addToMessage(toString((i + 1), f.get(i), customer));
                }
            }
        }
    ...
    public void displayHeaderForUsers(Flight flight, List<Customer> c) {
        User.addToMessage("Displaying Registered Customers for Flight No. \""+flight.getFlightNumber()+"\" %s \n\n");
        User.addToMessage("| SerialNum |   UserID   | Passenger Names | Age | EmailID | Home Address | Phone Number | Booked Tickets |");
        int size = flight.getListOfRegisteredCustomersInAFlight().size();
        for (int i = 0; i < size; i++) {
            User.addToMessage(toString(i, c.get(i), flightIndex(c.get(i).flightsRegisteredByUser, flight)));
        }
        User.showFlights(User.message);
    }
